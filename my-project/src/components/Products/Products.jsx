      ...
      <div className="grid grid-cols-5 gap-4">
        {products.map((product) => (
          <div key={product.id} className="bg-white p-4 rounded-md shadow-md">
            <img src={product.img} alt={product.name} className="w-28 h-28 object-cover mx-auto my-auto" />
            <h3 className="mt-2 font-semibold">{product.name.substring(0, 20) + "..."}</h3>
            <p className="text-gray-600">{product.description.substring(0, 20) + "..."}</p>
            <div className="flex justify-between mt-2">
              <p className="font-bold">{product.price}</p>
              <p className={`mt-2 mb-2 ${ product.discount === "10%"? "text-orange-500": "text-red-800"}`}>{product.discount}</p>
            </div>
            <p className="text-gray-500 line-through">{product.oldPrice}</p>
            <p
              className={`mt-2 mb-2 ${ product.rating === "Premium"? "text-orange-500": "text-gray-500"}`}>
              {product.rating}
            </p>
